        ...
        if (!enrollments) {
            ...
            forumEnrollments
              ...
              .map((enrollment) => {
                return {
                  ...
                  allowPrivateMessages: enrollment.users_permissions_user.allowPrivateMessages,
                };
              }),
            'id'
          );
          ...
          const privateChannels = await strapi.entityService.findMany(
            "api::channel.channel",
            {
              ...
              populate: {
                users_permissions_users: {
                  fields: ['id', 'username', 'name', 'lastname', 'allowPrivateMessages']
                }
              },
            }
          );
          ...
          const userPrivateChannels = privateChannels.filter((channel: any) => {
            return channel.users_permissions_users.length === 2 &&
                   channel.users_permissions_users.every((user: any) => user.allowPrivateMessages === true);
          }).map((channel: any) => {
            ...
            return {
              ...
              users_permissions_users: channel.users_permissions_users.map((user: any) => ({
                id: user.id,
                username: user.username,
                name: user.name,
                lastname: user.lastname,
              ...
              other_user: {
                ...
                avatar: otherUserAvatar && otherUserAvatar.avatar && otherUserAvatar.avatar.url 
    ...
    getUnreadCounts: async (ctx) => {
      ...
      try {
        ...
        for (const channel of channels) {
          ...
          const lastReadArr = await strapi.entityService.findMany('api::user-channel-read.user-channel-read', {
            filters: {
              user: userId,
          ...
          let unreadCount = 0;
          let lastReadOrder = 0;
          if (lastReadArr.length > 0 && lastReadArr[0].last_read_message && typeof lastReadArr[0].last_read_message.message_order === 'number') {
